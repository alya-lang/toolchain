...
# Upstream LLVM Clang releases for Darwin
LLVM_VERSION="18.1.8"
if [ "$ARCH" = "arm64" ] || [ "$ARCH" = "aarch64" ]; then
    ...
    ARCH="x64"
    UPSTREAM_URL="https://github.com/llvm/llvm-project/releases/download/llvmorg-${LLVM_VERSION}/clang+llvm-${LLVM_VERSION}-x86_64-apple-darwin.tar.xz"
fi

UPSTREAM_ARCHIVE="$TEMP_DIR/upstream.tar.xz"

echo "[1/4] Downloading minimal LLVM Darwin release for $ARCH..."
curl -fsSL "$UPSTREAM_URL" -o "$UPSTREAM_ARCHIVE"
...
# Strip non-essential compiler runtime
strip -u -r "$STAGING_DIR/bin/clang" 2>/dev/null || true
